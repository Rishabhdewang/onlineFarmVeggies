...
export class AuthService {
  ...
  verifyotp(data){
    return this.http.post(this.admin_base_url + "verifyOTP", data, { observe: "response" }).pipe(
      retry(3),
      catchError(this.errorHandler.handleError)
    );
  }
  ...
  resetPassword(data){
    return this.http.post(this.admin_base_url + "resetPassword", data, { observe: "response" }).pipe(
      retry(3),
      catchError(this.errorHandler.handleError)
    );
  }
